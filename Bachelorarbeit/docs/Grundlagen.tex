...
Imperfektionen können durch Fehler im Herstellungsprozess entstehen. Leitendes Material kann über sich selbst gefaltet sein und dadurch den Kontakt zwischen Elektrodenwickel und $Cap$, bzw. Gehäuse negativ beeinflussen. Um diesen Effekten entgegen zu wirken können Abschnitte des leitenden Teils der Elektrode entfernt werden wie in Abbildung \ref{fig:BottomConnectorTypes} gezeigt ist. Mögliche Vorgehensweisen sind die Entfernung von Material in festen Abständen \textbf{(a)} oder in größer werdenden Abständen \textbf{(b)}. Bei \textbf{(b)} kann die Anordnung so gewählt werden, dass nach der Wicklung der Elektrode die leitenden Teile ein gleichmäßiges Muster bilden.\footnote{Vgl. Fußnote \ref{cite:TeslaPatent}, S. 5, Tsuruta et al. 2020}\\
...
\begin{table}[h!]
	\caption{Beschriftung von Abbildung \ref{fig:BottomConnectorTypes}}
	\label{tab:BottomConnectorTypes}
	\vspace{0.2cm}	
	\begin{tabularx}{\textwidth}{ |X|X|  }
		\toprule[1.5pt]
		\textbf{Nummer} & \textbf{Beschreibung} \\
